...
public class VerificationPlat_Choisie
{
	private int numClient;
	private JFrame fenetre ; 
	private int recap ;
	  
	public VerificationPlat_Choisie(int numClient0,JFrame fenetre0,int recap0)		
	{
		recap = recap0 ; 
		...
		try
		{
			...
			try
			{
				...
				try
				{
					...
					ResultSet result = state.executeQuery("SELECT * FROM Plat WHERE NumClient ='"+numClient+"';");
					result.first();
					...
					try
					{
						...
						if(recap==1)
						{
							...
							{
								fenetre.dispose();
								(new TabletteEconomique(numClient,recap)).setVisible(true);
							}
							else if ( idclass == 1 ) 
							{
								fenetre.dispose();
								(new Tablette(numClient,recap)).setVisible(true);							
							}
						}
						if(recap==2)
						{
